...
_LIT(KDownloadAlipay,"Please download and install alipay plugin!");
// ============================ MEMBER FUNCTIONS ===============================
...
void CAppDemoAppView::OrderPay(const TDesC8& aSubject,
	...
	if(iInterface)
		...
		{
		      //暂不能支付，请安装支付宝安全支付服务插件
      NotifyUser(_L("Unable to pay, please download and install Alipay SecurePay plugin!"));
		}
	}
...
TBool CAppDemoAppView::CheckPlugin()
{
    CInterfaceDefinition* interface = NULL;
    TRAPD(error, interface = CInterfaceDefinition::NewL(KAliXPayOperationName));
    if(interface)
        delete interface;
    if(error == KErrNone)
        return ETrue;
     ...
     {
        //in here, you should download alipay.sisx and install it;
        NotifyUser(KDownloadAlipay);
     }
    else
    {
        //unknown error,notice user to restart app and retry
    }
    return EFalse;
}
 	...
 	{ 				 
 		...
 		if(verify&&plaint)
 		{
		    ...
		    TBool isSucc = IsSuccess(aResult);
 			...
 			retult->Des().Append(_L("  GetVerifyText fail  "));
